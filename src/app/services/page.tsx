...
const ServicesPage = () => {
  ...
  const categories = ["All", "AI", "Web", "App", "UI", "UX", "Design", "Development"];
  ...
  const filteredServices = useMemo(() => {
    return remainingServices.filter((service) => {
      ...
      const matchesSearch = service.title.toLowerCase().includes(searchQuery.toLowerCase()) || service.summary.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <>
      ...
      <div className="mb-16 text-center">
        ...
        <p className="text-gray-600 max-w-2xl mx-auto text-lg leading-relaxed">
          ...
          <br className="hidden sm:block"/> From AI to design, we've got you covered.
        </p>
      </div>

       {/* Featured Section */}
       {featuredService && selectedCategory === "All" && searchQuery === "" && (
        <div className="mb-20">
          <Link href={featuredService.fiverrUrl} target="_blank" rel="noopener noreferrer" className="group block relative rounded-3xl overflow-hidden aspect-video sm:aspect-[2/1] lg:aspect-[2.5/1] shadow-xl hover:shadow-2xl transition-shadow">
             <Image
              ...
              <div className="absolute bottom-0 left-0 p-6 sm:p-12 lg:p-16 max-w-4xl">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-white font-bold">{featuredService.rating.toFixed(1)}</span>
                    <span className="text-gray-300">({featuredService.reviews})</span>
                  </div>
                  <span className="text-white font-bold text-xl">{featuredService.price}</span>
                </div>
              </div>
          </Link>
        </div>
      ...
      {filteredServices.length === 0 && (
        <div className="text-center py-20">
            <p className="text-2xl text-gray-500 font-bold mb-2">No services found</p>
